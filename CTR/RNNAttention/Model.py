...
def main():
    ...
    parser.add_argument('-concat1Layer', action='store', dest='concat1Layer', default=[128])
    parser.add_argument('-concat2Layer', action='store', dest='concat2Layer', default=[128])
    parser.add_argument('-factor', action='store', dest='factor', default=128)
...
class Model:
    ...
    def add_model(self):
        ...
        with tf.name_scope('model'):
            ...
            # self.cell = rnn.MultiRNNCell([self.cell] * 1)
            ...
            outputs, last_state = tf.nn.dynamic_rnn(self.cell, itemHistss, initial_state=self.initial_state)
        ...
        with tf.name_scope("concat_rnnAttention"):
            ...
            preNextItem = tf.concat([preNextItem, last_state[0]] , 1)
    ...
    def nextDataInput(self, users , itemPoss=None , isTest=True):
        ...
        if isTest:
            for user in users:
                sl.append(int(min(item_num , len(self.dataSet.userHistDict[user]))))
            max_sl = int(max(sl))
            hist = np.zeros([len(users), max_sl], np.int32)
            for idx in range(len(users)):
                user = users[idx]
                userHists = self.dataSet.userHistDict[user]
                for index in range(sl[idx]):
                    hist[idx][index] = userHists[-(sl[idx] - index)]
                
        else:
            for user , itemPos in zip(users , itemPoss):
                sl.append(int(min(item_num , itemPos)))
            max_sl = int(max(sl))
            hist = np.zeros([len(users), max_sl], np.int32)
            for idx in range(len(users)):
                user = users[idx]
                userHists = self.dataSet.userHistDict[user]
                start = itemPoss[idx] - int(min(item_num , sl[idx]))
                end = itemPoss[idx]
                for index in range(start , end):
                    hist[idx][index - start] = userHists[index]
    ...
    def run_epoch(self, sess, verbose=10):
        ...
        for i in range(num_batches):
            ...
            userHists, histLen = self.nextDataInput(train_u_batch, train_pos_batch, isTest=False)

            feed_dict = self.create_feed_dict(train_u_batch, train_i_batch ,userHists , histLen, train_r_batch , drop=0.6)
